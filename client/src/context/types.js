export const ADD_JOB = "ADD_JOB";
export const GET_PROGRESS = "GET_PROGRESS";
export const GET_COMPLETED = "GET_COMPLETED";
export const ADD_TASKS = "ADD_TASKS";
export const ERROR = "ERROR";
export const GET_TASKS = "GET_TASKS";
export const SET_TIMEPASS = "SET_TIMEPASS";
export const ADD_CURRENT_TASK = "ADD_CURRENT_TASK";
export const SET_ID = "SET_ID";
export const DELETE_CURRENT_TASK = "DELETE_CURRENT_TASK";
export const CLEAR_CURRENT_TASKS = "CLEAR_CURRENT_TASKS";
export const SET_LOADING = "SET_LOADING";
export const SET_CURRENT_TASK = "SET_CURRENT_TASK";
export const UPDATE_CURRENT_TASK = "UPDATE_CURRENT_TASK";
export const SET_PAGE = "SET_PAGE";
export const EDIT_JOB = "EDIT_JOB";
export const EDIT_TASK = "EDIT_TASK";
export const SET_EDIT_JOB_STATE = "SET_EDIT_JOB_STATE";
export const CLEAR_JOB_EDIT_STATE = "CLEAR_JOB_EDIT_STATE";
export const COMPLETED_LOADING = "COMPLETED_LOADING";
export const MATERIALS_LOADING = "MATERIALS_LOADING";
export const SET_MATERIALS = "SET_MATERIALS";
export const SET_MATERIALS_LOADING = "SET_MATERIALS_LOADING";
export const DELETE_CURRENT_MATERIAL = "DELETE_CURRENT_MATERIAL";
export const EDIT_MATERIAL = "EDIT_MATERIAL";
export const UPDATE_MATERIAL = "UPDATE_MATERIAL";
export const ADD_MATERIAL = "ADD_MATERIAL";
export const SAVE_UPDATE = "SAVE_UPDATE";
export const SAVE_LOADER = "SAVE_LOADER";
export const SAVE_SERVERTYPE = "SAVE_SERVERTYPE";
export const UPDATE_CURRENT_ITEM = "UPDATE_CURRENT_ITEM ";
export const UPDATE_DATE = "UPDATE_DATE";
export const ADD_REMOVIE_LIST = "ADD_REMOVIE_LIST";
export const CALL_BACL_MATERIAL = "CALL_BACL_MATERIAL";
export const MATERIAL_ALERT = "MATERIAL_ALERT";
export const SERVER_ALERT = "SERVER_ALERT";
export const CLEAR_JOB_IN_PROCESS = "CLEAR_JOB_IN_PROCESS";
export const CLEAR_JOB_DATA = "CLEAR_JOB_DATA";
